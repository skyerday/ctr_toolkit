...
u16 u8_to_u16(u8 *value, u8 endianness)
{
	u16 new_value;
	switch(endianness){
		case(_BIG_ENDIAN): new_value =  (value[1]<<0) | (value[0]<<8); break;
		case(_LITTLE_ENDIAN): new_value = (value[0]<<0) | (value[1]<<8); break;
	}
	return new_value;
}
...
u32 u8_to_u32(u8 *value, u8 endianness)
{
	u32 new_value;
	switch(endianness){
		case(_BIG_ENDIAN): new_value = (value[3]<<0) | (value[2]<<8) | (value[1]<<16) | (value[0]<<24); break;
		case(_LITTLE_ENDIAN): new_value = (value[0]<<0) | (value[1]<<8) | (value[2]<<16) | (value[3]<<24); break;
	}
	return new_value;
}
